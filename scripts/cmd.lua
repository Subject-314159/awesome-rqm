...
test.register_commands = function()
    ...
    commands.add_command("rqm_reset", "Force a re-init", function(command)
        init(command)
        game.print("[RQM] Reset complete")
        log("(" .. game.tick .. ") Reset complete")
        local p = game.get_player(command.player_index)
        local f = p.force
    ...
    commands.add_command("rqm_dump", "Dumps the data in the log", function(command)
        local p = game.get_player(command.player_index)
        local f = p.force
        log("===== technology =====")
        log(serpent.block(storage.state.forces[f.index].technology))
        log("===== queue =====")
        log(serpent.block(storage.forces[f.index].queue))
        game.print("Dumped mod data to log")
        log("dump complete")
    end)
